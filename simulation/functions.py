...
def panaderia_main_func(desde, cantidad_mostrar, total):
    fin = {}
    llaves = {}
    lista_clientes = []
    # INICIALIZACION -------------------------------------------------------------------
    cont_filas = 0
    cont_clientes = 0
    cont_clientes_abandono = 0
    rta = {}
    llegada_cliente = LlegadaCliente(id=1,id_cliente=0, valor_formula=3)
    cola_panadero= [[], []]
    lista_panaderos = []
    dicc = {"reloj": 0,
            }
    llegada_cliente.calc_valor_reloj(reloj=dicc["reloj"])
    dict_metricas = {
                     'acum_clientes': 1,
